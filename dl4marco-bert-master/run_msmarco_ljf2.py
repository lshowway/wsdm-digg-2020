...
flags.DEFINE_string(
    "data_dir",
    "D:/backup/wsdm_cup/bert_tfrecord/",
    "The input data dir. Should contain the .tfrecord files and the supporting "
    "query-docids mapping files.")

flags.DEFINE_string(
    "bert_config_file",
    "D:/backup/wsdm_cup/bert_conf/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string(
    "output_dir", "D:/backup/wsdm_cup/bert_out_bm25_neg/",
...
flags.DEFINE_string(
    "init_checkpoint",
    "D:/backup/wsdm_cup/bert_conf/bert_model.ckpt",
...
flags.DEFINE_integer("eval_batch_size", 29, "Total batch size for eval.")
...
flags.DEFINE_integer("num_eval_docs", 20,
    ...
    #for set_name in ["dev"]:
      ...
      if FLAGS.msmarco_output:
        ...
        msmarco_file = tf.gfile.Open(
            FLAGS.output_dir + "/msmarco_predictions_" + str(FLAGS.num_eval_docs) + "-" + r + "-" +  set_name + ".tsv", "w")

        msmarco_file_score = tf.gfile.Open(
            FLAGS.output_dir + "/msmarco_predictions_" + str(FLAGS.num_eval_docs)+ "-" + r + "-" + set_name + ".score", "w")
      ...
      for item in result:
        ...
        if len(results) == FLAGS.num_eval_docs:
          ...
          eval_results.append(pred_docs)
          all_metrics += metrics.metrics(gt=gt, pred=pred_docs, metrics_map=METRICS_MAP)
          ...
          if FLAGS.msmarco_output:
            ...
            for doc_idx in pred_docs:
              ...
              doc_score = scores[doc_idx]
              # Skip fake docs, as they are only used to ensure that each query
              # has 1000 docs.
              if doc_id != "00000000":
                tmp.append(doc_id)
                tmp_score.append(str(doc_score))

            msmarco_file.write(','.join(tmp) + '\n')
            msmarco_file_score.write(','.join(tmp_score) + '\n')
...
srun --gres=gpu:1  python3 run_msmarco.py  \
  --data_dir=/home/LAB/zhaoqh/ljf/datasets/bert_tfrecord  \
  --bert_config_file=/home/LAB/zhaoqh/workspace/CitationIntent/uncased_L-12_H-768_A-12/bert_config.json \
  --init_checkpoint=/home/LAB/zhaoqh/workspace/CitationIntent/uncased_L-12_H-768_A-12/bert_model.ckpt  \
  --output_dir=/home/LAB/zhaoqh/ljf/datasets/bert_output/  \
  --msmarco_output=True \
  --do_train=True  \
  --do_eval=True  \
  ...
  srun --gres=gpu:1  python3 run_msmarco_ljf2.py  \
  --data_dir=/home/LAB/zhaoqh/ljf/datasets/bert_tfrecord  \
  --bert_config_file=/home/LAB/zhaoqh/workspace/CitationIntent/uncased_L-12_H-768_A-12/bert_config.json \
  --init_checkpoint=/home/LAB/zhaoqh/workspace/CitationIntent/uncased_L-12_H-768_A-12/bert_model.ckpt  \
  --output_dir=/home/LAB/zhaoqh/ljf/datasets/bert_output_bm25_neg/  \
  --msmarco_output=True \
  --do_train=True  \
  --do_eval=True  \
  --num_train_steps=200000 \
  ...
  --bert_config_file=/home/LAB/zhaoqh/workspace/CitationIntent/uncased_L-12_H-768_A-12/bert_config.json \
  --init_checkpoint=/home/LAB/zhaoqh/workspace/CitationIntent/uncased_L-12_H-768_A-12/bert_model.ckpt  \
  --output_dir=/home/LAB/zhaoqh/ljf/datasets/bert_output_top50/  \
  --msmarco_output=True \
  --do_train=False  \
  --do_eval=True  \
  --num_train_steps=200000 \
